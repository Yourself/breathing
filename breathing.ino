...
This is the code for the AirGradient DIY PRO Air Quality Sensor with an ESP8266 Microcontroller with the SGP40 TVOC
module from AirGradient.
...
void updateTVOC() {
    ...
    if (conditioning_s > 0) {
        ...
        if (sgp41.measureRawSignals(compensationRh, compensationT, srawVoc, srawNox)) {
            TVOC = -1;
            NOX = -1;
            return;
        }
...
void updateOLED() {
    if (currentMillis - previousOled >= oledInterval) {
        ...
        String ln2 = "TVOC:" + (TVOC < 0 ? String("N/A") : String(TVOC)) + " NOX:" + (NOX < 0 ? String("N/A") : String(NOX));
...
void sendToServer() {
    if (currentMillis - previoussendToServer >= sendToServerInterval) {
        ...
        String payload =
            "{\"wifi\":" + String(WiFi.RSSI()) + (Co2 < 0 ? "" : ", \"rco2\":" + String(Co2)) +
            (pm25 < 0 ? "" : ", \"pm02\":" + String(pm25)) + (TVOC < 0 ? "" : ", \"tvoc_index\":" + String(TVOC)) +
            (NOX < 0 ? "" : ", \"nox_index\":" + String(NOX)) + (isnan(temp) ? "" : ", \"atmp\":" + String(temp)) +
